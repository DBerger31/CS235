...
*/
// returns a sorted list of items in increasing order

template<class T>
List<T> List<T>::scanSublist(size_t position){
   Node<T>* temp_ptr = getPointerTo(position); // creates a pointer to the starting position
   Node<T>* move = temp_ptr; // traveling pointer
   List<T> sub;

   if (temp_ptr == nullptr) { // if the list is empty
      return sub;
   }

   /* 1. travel left -- stop once the begining of the list is hit 
   or if you find a number that is greater than the number to the right
   2. add the first/lowest number
   3/ travel right -- stop pnce the end of the list is hit or
   if you find a number that is less than the number to the left (adding the numbers along the way)
   */

   else {
      while (((move->getPrevious()) != nullptr) && ((move->getItem()) >= (move->getPrevious())->getItem())){
         move = move->getPrevious();
      }
      int i = 0; // used to keep track of position
      sub.insert(i, move->getItem());
      while ((move->getNext() != nullptr) && (move->getItem()) <= move->getNext()->getItem()) {
         i++;
         move = move->getNext();
         sub.insert(i, move->getItem());
