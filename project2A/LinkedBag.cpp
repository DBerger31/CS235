...
LinkedBag<T> LinkedBag<T>::bagUnion(const LinkedBag<T>& a_bag) const {
	LinkedBag<T> unionbag(a_bag); // creates a bag with the contents of a_bag
	Node<T>* temp_ptr = head_ptr_; // creates a traveling pointer
	for (int i = 0; i < getCurrentSize(); i++){
		unionbag.add(temp_ptr->getItem()); // adds the item
		//cout << "ADDED ITEM : " << temp_ptr->getItem() << endl;
		temp_ptr = temp_ptr->getNext(); // sets that pointer to point to the next pointer
	}
	return unionbag;
}
...
LinkedBag<T> LinkedBag<T>::bagIntersectionNoDuplicates(const LinkedBag<T>& a_bag) const{
	...
	while(temp_ptr != nullptr) {
		if (a_bag.contains(temp_ptr->getItem()) && !interbag.contains(temp_ptr->getItem())){
			interbag.add(temp_ptr->getItem());
			temp_ptr = temp_ptr->getNext();
		}
		else { temp_ptr = temp_ptr->getNext(); }
	}
...
LinkedBag<T> LinkedBag<T>::bagDifference(const LinkedBag<T>& a_bag) const {
	...
	Node<T>* temp_ptr = head_ptr_;
	while(temp_ptr != nullptr) {
		//if diffbag doesnt already contain the item and a_bag doesnt have it add it to the bag
	...
	Node<T>* a_temp_ptr = a_bag.head_ptr_;
	while(a_temp_ptr != nullptr) {
		// if diffbag doesnt already contain the item and its not in the original then add the item
...
void LinkedBag<T>::operator= (const LinkedBag<T>& a_bag){
	...
	clear(); // clears the og bag
	Node<T>* a_temp_ptr = a_bag.head_ptr_; // create a  temp pointer for a_bag
	while(a_temp_ptr != nullptr){ //while its not null
		add(a_temp_ptr->getItem()); //add the node to the bag
		a_temp_ptr = a_temp_ptr->getNext(); //get the next node to add
	}
}
...
bool LinkedBag<T>::addToEnd(const T& new_entry){
	Node<T>* entry_ptr = new Node<T>(new_entry); // dynamically allocate memory for the new entry
	Node<T>* temp_ptr = head_ptr_; // points to what head ptr points to
	...
	return ((temp_ptr->getItem()) == new_entry); // if it worked return true
}
